            ...
            std::vector<std::string> keys = opts.get_engine_args().get_option_keys();
                    foreach(std::string opt, keys) {
                            if (opt == "timeout") {
                                ...
                                if (rmi.procid() == 0)
                                    logstream(LOG_EMPH) << "Engine Option: timeout = " << timed_termination
                                                        << std::endl;
                            } else if (opt == "factorized") {
                                opts.get_engine_args().get_option("factorized", factorized_consistency);
                                if (rmi.procid() == 0)
                                    logstream(LOG_EMPH) << "Engine Option: factorized = " << factorized_consistency
                                                        << std::endl;
                            } else if (opt == "nfibers") {
                                ...
                                if (rmi.procid() == 0)
                                    logstream(LOG_EMPH) << "Engine Option: track_task_time = " << track_task_time
                                                        << std::endl;
        ...
        void eval_sched_task(const lvid_type lvid,
            ...
            if (rec.owner != rmi.procid()) {
                ...
                for (size_t i = 0; i < gather_futures.size(); ++i) {
                    ...
                    cachelocks[lvid * rmi.numprocs() + mirror].lock();
                    remote_gather_cache[lvid * rmi.numprocs() + mirror] = gather_futures[i]().first;
